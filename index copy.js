...
//py related
const remote_url = 'https://nowsecure.nl'

const pySessionId = Math.random() * 1000;
// Create our request client & API executor
const client = new HttpClient(remote_url);
const executor = new Executor(client);
// const pySession = new selenium.Session(pySessionId, {});
// const remoteDriver = new WebDriver(pySession, executor);
...
const crawlPage = async () => {
    ...
    console.log(url[currentUrlIndex])
    return new Promise(async (res, rej) => {
        await driver.get(url[currentUrlIndex]);
        const links = await findAllLinkTags();
        if (links.length > 0) {
...
const findAllLinkTags = () => {
    return new Promise(async (res, rej) => {
        let links = await driver.findElements(By.tagName('a'));
        let linksWithMatchingHref = []
        for (let link of links) {
            let href = await link.getAttribute("href");
            if (typeof href === "string") {
                href = href?.toLowerCase();
                console.log("PRINTING", href)
                console.log('is a match?', targetList.some(target => href.includes(target)))
                if (targetList.some(target => href.includes(target))) {
                    linksWithMatchingHref.push(href);
                }
            }

        }
        res(linksWithMatchingHref)
    })
